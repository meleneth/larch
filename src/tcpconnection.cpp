...
int TCPConnection::has_data_waiting(void) { return !lines.empty(); }
...
void TCPConnection::sendall(std::string cmd) {
  ...
  while (total < num_bytes) {
    ...
    if (n == -1) {
      LOG(INFO) << "Network error on send?  Oh I'm so scared :/";
      break;
    }
    if (n == 0) {
      LOG(INFO) << "I'm pretty sure I used to crash here";
      connected = 0;
      total = num_bytes + 1;
    }
    total += n;
...
unsigned int TCPConnection::read_packets(void) {
  // will only be called when we have data waiting
  if (!connected) return 0;

  if ((numbytes = recv(sockfd, buf + buf_end_pos, MAXDATASIZE - buf_end_pos,
                       0)) == -1) {
    connected = 0;
    return 0;
  }
...
unsigned int TCPConnection::get_krate() {
  ...
  if (ticks) {
    if (bytes_since_last_time) {
      krate = (krate + ((bytes_since_last_time / ticks) / (1024 / 10))) / 2;
      bytes_since_last_time = 0;
    } else {
      krate = 0;
    }
...
unsigned int TCPConnection::num_ticks(struct timeval *now) {
  return ((now->tv_sec - last_time.tv_sec) * 10) +
         (now->tv_usec / MS_PER_TICK) - (last_time.tv_usec / MS_PER_TICK);
}
